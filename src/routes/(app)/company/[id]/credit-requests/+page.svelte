<script lang="ts">
	...
	async function loadRequests() {
		loading = true;
		try {
			const res = await getCreditRequests({ companyId: Number($page.params.id), page: currentPage, size: 20 });
...
<PageHeader title="Credit Requests" description="Review and manage credit requests" />

{#if loading}
	<LoadingSpinner />
{:else}
	<Table headers={['ID', 'Employee', 'Amount', 'Reason', 'Status', 'Created', 'Actions']}>
		{#each requests as req}
			<tr>
				<td class="px-4 py-3">{req.id}</td>
				<td class="px-4 py-3 font-medium">{req.userName}</td>
				<td class="px-4 py-3">{req.amount}</td>
				<td class="px-4 py-3 max-w-xs truncate">{req.reason}</td>
